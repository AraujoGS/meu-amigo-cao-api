...
describe('CheckAccountByEmailPostgres Repository', () => {
  ...
  it('should CheckAccountByEmailPostgresRepository return true if account exists', async () => {
    const sut = makeSut()
    const params = await mockAccount()
    const response = await sut.check(params.email)
    expect(response).toBe(true)
  })
  it('should CheckAccountByEmailPostgresRepository return false if account not exists', async () => {
    const sut = makeSut()
